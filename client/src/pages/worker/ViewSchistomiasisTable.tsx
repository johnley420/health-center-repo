...
const ViewSchistosomiasisTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8081/schistosomiasis/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await axios.get(`http://localhost:8081/schistosomiasis-form/${client.id}`);
