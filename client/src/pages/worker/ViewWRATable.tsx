...
const ViewWRATable = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await fetch(`http://localhost:8081/wra-family-planning/clients?worker_id=${workerId}&category_name=${encodeURIComponent("Family Planning")}`);
        if (!response.ok) {
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/wra/${client.id}`);
      if (response.ok) {
