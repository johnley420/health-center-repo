...
import { IoSearch } from "react-icons/io5"; // Adjust the import path as necessary
import { ClientType } from '../../types'; // Adjust the import path as necessary
import { WRAForm } from "../../components/modals/forms/WRA15-49";

const ViewWRATable = () => {
  // Ensure ClientType is imported or defined
  // type ClientType = {
  //   id: number;
  //   worker_id: string;
  //   fname: string;
  ...
  // Fetch WRA client data from the server
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await fetch(`https://health-center-repo-production.up.railway.app/wra-family-planning/clients?worker_id=${workerId}&category_name=${encodeURIComponent("Family Planning")}`);
        if (!response.ok) {
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`https://health-center-repo-production.up.railway.app/wra/${client.id}`);
      if (response.ok) {
  ...
  return (
    <div className="w-full p-2">
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-7">
        <h1 className="text-xl md:text-3xl font-bold pl-3">WRA 15-49 Family Planning</h1>
        <Input
          size="lg"
          label="Search"
      ...
      <Table isStriped aria-label="WRA client data table">
        <TableHeader>
