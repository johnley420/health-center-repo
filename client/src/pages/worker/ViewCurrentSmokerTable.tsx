...
const ViewCurrentSmokerTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await axios.get(
          `health-center-repo-production.up.railway.app/currentsmokers/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
            "Current Smokers"
          )}`
        );
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await axios.get(`health-center-repo-production.up.railway.app/currentsmokers-form/${client.id}`);
