...
import { CurrentSmokerForm } from "../../components/modals/forms/CurrentSmokers";

const ViewCurrentSmokerTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await axios.get(
          `https://https://health-center-repo-production.up.railway.app/currentsmokers/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
            "Current Smokers"
          )}`
        );
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await axios.get(`https://https://health-center-repo-production.up.railway.app/currentsmokers-form/${client.id}`);
  ...
  return (
    <div className="w-full p-2">
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-7">
        <h1 className="text-xl md:text-3xl font-bold pl-3">Current Smokers</h1>
        <Input
          size="lg"
          label="Search"
      ...
      <Table isStriped aria-label="Current Smokers data table">
        <TableHeader>
        ...
        <TableBody>
          {filteredClients.map((client, index) => (
            <TableRow key={client.id}>
              ...
              <TableCell className="text-base text-black py-4 pl-4">
                {new Date(client.date_registered).toLocaleDateString()} {/* Format the date */}
              </TableCell>
...
export default ViewCurrentSmokerTable;