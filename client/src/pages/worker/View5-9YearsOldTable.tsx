...
const View5to9YearsOldTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await fetch(
          `https://health-center-repo-production.up.railway.app/5-9yearsold/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`https://health-center-repo-production.up.railway.app/5-9yearsold-form/${client.id}`);
      if (response.ok) {
