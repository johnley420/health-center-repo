...
import { FIVETO9CHILDRENFORM } from "../../components/modals/forms/5-9Children";


const View5to9YearsOldTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await fetch(
          `https://https://health-center-repo-production.up.railway.app/5-9yearsold/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`https://https://health-center-repo-production.up.railway.app/5-9yearsold-form/${client.id}`);
      if (response.ok) {
  ...
  return (
    <div className="w-full p-2">
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-7">
        <h1 className="text-xl md:text-3xl font-bold pl-3">5-9 Years old children (School aged children)</h1>
        <Input
          size="lg"
          label="Search"
      ...
      <Table isStriped aria-label="5-9 years old children data table">
        <TableHeader>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
        ...
        >
          ...
          >
            <FIVETO9CHILDRENFORM data={selectedClient} selectedData={selectedData} />
          </ModalBody>
          <ModalFooter>
...
export default View5to9YearsOldTable;
