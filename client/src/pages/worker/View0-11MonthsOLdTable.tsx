...
const View0to11MonthsOLdTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await fetch(
          `https://health-center-repo-production.up.railway.app/0-11monthsold/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`https://health-center-repo-production.up.railway.app/0-11monthsold-form/${client.id}`);
      if (response.ok) {
