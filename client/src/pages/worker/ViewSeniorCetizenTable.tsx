...
const ViewSeniorCitizenTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await fetch(
          `health-center-repo-production.up.railway.app/senior-citizen/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
            "Senior Citizen"
          )}`
        );
        if (!response.ok) {
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`health-center-repo-production.up.railway.app/senior-citizen-form/${client.id}`);
      if (response.ok) {
