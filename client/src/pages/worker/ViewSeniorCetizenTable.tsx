...
import { SeniorCitizenForm } from "../../components/modals/forms/Senior Citizen";


const ViewSeniorCitizenTable = () => {
  ...
  // Fetch Senior Citizen clients data from the server
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await fetch(
          `https://https://health-center-repo-production.up.railway.app/senior-citizen/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
            "Senior Citizen"
          )}`
        );
        if (!response.ok) {
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`https://https://health-center-repo-production.up.railway.app/senior-citizen-form/${client.id}`);
      if (response.ok) {
  ...
  return (
    <div className="w-full p-2">
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-7">
        <h1 className="text-xl md:text-3xl font-bold pl-3">Senior Citizen (60 Years old and above)</h1>
        <Input
          size="lg"
          label="Search"
      ...
      <Table isStriped aria-label="Senior Citizen client data table">
        <TableHeader>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
        ...
        >
          ...
          >
            <SeniorCitizenForm data={selectedClient} selectedData={selectedData} />
          </ModalBody>
          <ModalFooter>
...
export default ViewSeniorCitizenTable;
