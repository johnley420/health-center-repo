...
const ViewHypertensiveTable = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        ...
        const response = await fetch(`health-center-repo-production.up.railway.app/hypertensive/clients?worker_id=${workerId}&category_name=${encodeURIComponent(categoryName)}`);
        if (!response.ok) {
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`health-center-repo-production.up.railway.app/hypertensive-form/${client.id}`);
      if (response.ok) {
