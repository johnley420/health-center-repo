...
import { HypertensiveForm } from "../../components/modals/forms/Hypertensive";


const ViewHypertensiveTable = () => {
  ...
  // Fetch hypertensive client data from the server
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const categoryName = "Hypertensive And Type 2 Diabetes";
        const response = await fetch(`https://https://health-center-repo-production.up.railway.app/hypertensive/clients?worker_id=${workerId}&category_name=${encodeURIComponent(categoryName)}`);
        if (!response.ok) {
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`https://https://health-center-repo-production.up.railway.app/hypertensive-form/${client.id}`);
      if (response.ok) {
        ...
        // If no data found, open the form modal
        setIsOpen(true);
      }
    } catch (error) {
  ...
  return (
    <div className="w-full p-2">
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-7">
        <h1 className="text-xl md:text-3xl font-bold pl-3">Hypertensive And Type 2 Diabetes</h1>
        <Input
          size="lg"
          label="Search"
      ...
      <Table isStriped aria-label="Hypertensive client data table">
        <TableHeader>
...
export default ViewHypertensiveTable;
