...
import { TentoNineteenForm } from "../../components/modals/forms/10-19OLD";

// Import TentoNineteenForm as a named import


const View10to19yotable = () => {
  ...
  // Fetch 10-19 Years Old clients data from the server
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await fetch(
          `https://health-center-repo-production.up.railway.app/10-19yo/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`https://health-center-repo-production.up.railway.app/10-19yo-form/${client.id}`);
      if (response.ok) {
  ...
  return (
    <div className="w-full p-2">
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-7">
        <h1 className="text-xl md:text-3xl font-bold pl-3">10-19 Years old (Adolescents)</h1>
        <Input
          size="lg"
          label="Search"
      ...
      <Table isStriped aria-label="10-19 Years Old client data table">
        <TableHeader>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
        ...
        >
          ...
          >
            {/* Use the named export in the component */}
            <TentoNineteenForm data={selectedClient} selectedData={selectedData} />
          </ModalBody>
          <ModalFooter>
...
export default View10to19yotable;
