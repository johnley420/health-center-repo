...
const View10to19yotable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await fetch(
          `http://localhost:8081/10-19yo/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/10-19yo-form/${client.id}`);
      if (response.ok) {
