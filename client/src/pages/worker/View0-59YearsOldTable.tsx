...
const View0to59YearsOldTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8081/0-59yearsold/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await axios.get(`http://localhost:8081/zero_to_fiftynine-form/${client.id}`);
