// src/components/modals/forms/0-59OLD/View0to59YearsOldTable.tsx
...
import { ZEROTO59MONTHFORM } from "../../components/modals/forms";

const View0to59YearsOldTable = () => {
    ...
    const fetchClients = async () => {
      try {
        const response = await axios.get(
          `https://health-center-repo-production.up.railway.app/0-59yearsold/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await axios.get(`https://health-center-repo-production.up.railway.app/zero_to_fiftynine-form/${client.id}`);
  ...
  return (
    <div className="w-full p-2">
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-7">
        <h1 className="text-xl md:text-3xl font-bold pl-3">0-59 Years Old Screened for Visual Activity</h1>
        <Input
          size="lg"
          label="Search"
      ...
      <Table isStriped aria-label="0-59 Years Old Screened for Visual Activity data table">
        <TableHeader>
          ...
          {/* Removed Date Registered Column */}
...
export default View0to59YearsOldTable;
