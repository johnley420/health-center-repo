...
import { FilariasisForm } from "../../components/modals/forms/Filariasis";


const ViewFilariasisTable = () => {
  ...
  // Fetch Filariasis clients data from the server
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await fetch(
          `https://https://health-center-repo-production.up.railway.app/filariasis/clients?worker_id=${workerId}&category_name=${encodeURIComponent(
  ...
  const handleAddForm = async (client: ClientType) => {
    ...
    try {
      const response = await fetch(`https://https://health-center-repo-production.up.railway.app/filariasis-form/${client.id}`);
      if (response.ok) {
  ...
  return (
    <div className="w-full p-2">
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-7">
        <h1 className="text-xl md:text-3xl font-bold pl-3">Filariasis Program Services</h1>
        <Input
          size="lg"
          label="Search"
      ...
      <Table isStriped aria-label="Filariasis client data table">
        <TableHeader>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
        ...
        >
          ...
          >
            <FilariasisForm data={selectedClient} selectedData={selectedData} />
          </ModalBody>
          <ModalFooter>
...
export default ViewFilariasisTable;
