...
import AddMedicineForm from '../../components/modals/AddMedicineForm';
import EditMedicineModal from '../../components/modals/EditMedicineForm';
...
const Inventory: React.FC = () => {
  ...
  const handleConfirmDelete = () => {
    setMedicines(medicines.filter((med) => med.id !== (selectedMedicine?.id || 0)));
  ...
  return (
    <div className="p-6">
      ...
      <Input className="w-96" placeholder="Search by name or category" onChange={(e) => setSearchQuery(e.target.value)} />
        ...
        <TableBody>
          {filteredMedicines.map((medicine) => (
            <TableRow key={medicine.id}>
              <TableCell>{<img  src={medicine.image} className="w-20"  />}</TableCell>
              <TableCell>{medicine.name}</TableCell>
              <TableCell>{medicine.category}</TableCell>
              <TableCell>{medicine.quantity}</TableCell>
              <TableCell>{medicine.expirationDate}</TableCell>
              <TableCell>
                <Button color="warning" className="mr-4" onClick={() => handleEditMedicine(medicine)}>Edit</Button>
