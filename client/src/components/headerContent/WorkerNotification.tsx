...
const WorkerNotification: React.FC<WorkerNotificationProps> = ({ workerId }) => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await axios.get(
          `https://health-center-repo-production.up.railway.app/notifications?worker_id=${workerId}`
        );
  ...
  const handleMarkAsRead = async () => {
    try {
      await axios.put("https://health-center-repo-production.up.railway.app/notifications/mark-as-read", {
        worker_id: workerId,
      });
      setUnreadCount(0);
      setNotifications([]);
