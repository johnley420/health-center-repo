...
const UpdateAccount: React.FC<PropsType> = ({ isOpen, onClose, data, onUpdate }) => {
  ...
  const validateAdmin = async (adminPassword: string): Promise<boolean> => {
    try {
      ...
      const response = await axios.post("https://https://health-center-repo-production.up.railway.app/validate-admin", {
        adminPassword,
        sessionRole,
      });
  ...
  const updateUser = async (userId: number, username: string, password: string) => {
    try {
      ...
      const response = await axios.put(`https://https://health-center-repo-production.up.railway.app/update-user/${userId}`, {
        username,
        password,
      });
